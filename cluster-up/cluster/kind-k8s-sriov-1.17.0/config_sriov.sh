...
NUM_PFS_REQUIRED=${NUM_PFS_REQUIRED:-1}

OPERATOR_GIT_HASH=8d3c30de8ec5a9a0c9eeb84ea0aa16ba2395cd68  # release-4.4
SRIOV_OPERATOR_NAMESPACE="sriov-network-operator"

if [ $NUM_PFS_REQUIRED -le 0 ]; then
  echo "FATAL: Wrong value of NUM_PFS_REQUIRED, must be a positive integer, less or equal to the actual number of PFs available"
  exit 1
fi
...
function move_sriov_pfs_netns_to_node {
  ...
  if [ "${#pf_array[@]}" -lt "$num_pfs_required" ]; then
    echo "FATAL: Could not allocate enough PFs, please check PF_BLACKLIST, NUM_PFS_REQUIRED, and how many PF actually available or used already"
    exit 1
  fi
...
NODE_PFS=($(move_sriov_pfs_netns_to_node "$SRIOV_NODE" "$NUM_PFS_REQUIRED"))
